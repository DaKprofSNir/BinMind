...
void setup(){
    ...
    PORTB = 0x3F; // 0011 1111 on allume tout
    ...
    UBRR0L = 0x68; // 9600Bds (SysClock / (16 * bauds)) -1
    UBRR0H = 0x00; // 9600Bds (SysClock / (16 * bauds)) -1
    UCSR0B = 0x18; // Autoriser Réception + Emission
...
void loop(){
  ...
  while( not win ){
    ...
    PORTB = nombre; delay(25); // les 1 vont briller au max
    
    //---- lire la réponse du joueur dans l'USART
    if( (UCSR0A & 0x80) != 0 ){ // on a reçu un carectère
